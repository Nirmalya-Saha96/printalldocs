...
var data = JSON.stringify({
  ...
  "name_check":nameCH,
  "address_check": addressCH,
  "photo_check": photoCH,
  "boimetric_check": bioCH
});
...
if(nameCH&&addressCH&&photoCH&&bioCH)
{
   axios(config)
.then(function (response) {
  //console.log(JSON.stringify(response.data));
  toast.success("Successfully Linked",{autoClose:2000})
})
.catch(function (error) {
   ...
   toast.error("Please Check all the checkboxes",{autoClose:2000})
}
    ...
    return (
        <div className="row w-75 m-auto">
             <div className="mb-3 mt-4 col-md-4">
                <label className="form-label text-secondary">Aadhar Card No.</label>
                <input type="text" className="form-control" value={aadhar} onChange={(e)=>{setAadhar(e.target.value)}} />
             </div>
             <div className="mb-3 mt-4 col-md-4">
                <label className="form-label text-secondary">Voter Card No.</label>
